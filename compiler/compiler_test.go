...
func TestTestIntegerArithmetic(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             "1 + 2",
			expectedConstants: []interface{}{1, 2},
...
func TestConditionals(t *testing.T) {
	runCompilerTest(t, []compilerTestCase{
		{
			input: `
			if (true) { 10 }; 3333;
			`,
...
func runCompilerTest(t *testing.T, tests []compilerTestCase) {
	...
	for _, tt := range tests {
		...
		if err := testInstruction(tt.expectedInstructions, bytecode.Instructions); err != nil {
			t.Fatalf("testInstructions fail: %s", err)
		}
		if err := testConstants(t, tt.expectedConstants, bytecode.Constants); err != nil {
			t.Fatalf("testConstants fail: %s", err)
		}
	}
}
...
func testConstants(
	...
	for i, constant := range expected {
		...
		case []code.Instructions:
			...
			if err := testInstruction(constant, fn.Instructions); err != nil {
